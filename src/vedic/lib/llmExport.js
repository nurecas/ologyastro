...
import { aspectMap, rashiAspectMap } from '../compute/drishti.js';
import { antarSequence, currentMaha, currentAntar, currentPratyantar, dashaAtDate } from '../compute/dasha.js';
...
const SCHEMA = {
  ...
  chart: {
    ...
    dasha: 'Vimshottari tree seeded by Moon\'s nakshatra: running_lord, current_maha, current_antar, current_pratyantar, next_5_mahadashas, antardashas_in_current_maha. All dates ISO 8601.',
...
function buildChart(state) {
  ...
  const next5 = c.dasha.sequence.slice(0, 5).map(m => ({
    lord: m.lord,
    years: m.years,
  ...
  return {
    ...
    arudha_lagna: c.arudha
      ? { sign: c.arudha.sign, sign_sa: c.arudha.signSa, sign_idx: c.arudha.signIdx, house: c.arudha.house }
      : null,
    upapada_lagna: c.upapada
      ? { sign: c.upapada.sign, sign_sa: c.upapada.signSa, sign_idx: c.upapada.signIdx, house: c.upapada.house }
      : null,
    vargas: VARGA_INFO.map(v => {
      ...
      return {
        ...
        sa: v.sa,
        area: v.area,
        lagna: { sign: dv.lagnaSign, sign_sa: dv.lagnaSignSa, sign_idx: dv.lagnaSignIdx },
        planets: dv.planets.map(p => ({
          name: p.name, sign: p.sign, sign_idx: p.signIdx,
    ...
    dasha: {
      ...
      current_pratyantar: (() => {
        ...
        return p ? { lord: p.lord, years: p.years, start: p.start.toISOString(), end: p.end.toISOString() } : null;
      })(),
      next_mahadashas: next5,
    ...
    yogas: (c.yogas || []).map(y => ({
      id: y.id, name: y.name, reason: y.reason, blurb: y.blurb, strength: y.strength,
    })),
    doshas: (() => {
      const d = c.doshas || {};
      return {
        kala_sarpa: d.kalaSarpa ? { name: d.kalaSarpa.name, severity: d.kalaSarpa.severity, reason: d.kalaSarpa.reason, blurb: d.kalaSarpa.blurb } : null,
        mangal:     d.mangal    ? { name: d.mangal.name,    severity: d.mangal.severity,    reason: d.mangal.reason,    blurb: d.mangal.blurb    } : null,
        combust:    d.combust   || [],
      };
    })(),
    sade_sati: (() => {
      const sp = computeSaturnPeriods(c, { now });
      if (!sp) return null;
      return {
        ...
        all_periods: sp.sadeSati.map(p => ({
          ...
          phases: p.phases.map(ph => ({
            phase: ph.label, sign: ph.signName,
            start: ph.start?.toISOString() || null,
            end:   ph.end?.toISOString() || null,
          })),
        })),
        ashtama_shani: sp.ashtamaShani.map(p => ({ start: p.start.toISOString(), end: p.end.toISOString() })),
        ardha_ashtama_shani: sp.ardhaAshtamaShani.map(p => ({ start: p.start.toISOString(), end: p.end.toISOString() })),
      };
    })(),
    argala: (c.argala || []).map(row => ({
      house: row.house, sign: row.signName,
      primary: row.primary.map(a => ({
        kind: a.kind, sign: a.signName, house: a.house,
        planets: a.planets,
        virodha_sign: a.virodhaSignName, virodha_house: a.virodhaHouse,
    ...
    ashtakavarga: c.ashtakavarga ? {
      ...
      rows: c.ashtakavarga.rows.map(r => ({
        house: r.house, sign: r.signName, sav: r.sav, bav: r.bav,
      })),
    } : null,
    upagrahas: c.upagrahas ? {
      ...
      gulika: c.upagrahas.gulika ? {
        sign: c.upagrahas.gulika.sign, sign_idx: c.upagrahas.gulika.signIdx,
        within_deg: c.upagrahas.gulika.withinDeg, house: c.upagrahas.gulika.house,
        nakshatra: c.upagrahas.gulika.nakshatra, nakshatra_lord: c.upagrahas.gulika.nakshatraLord,
        pada: c.upagrahas.gulika.pada,
      } : null,
      mandi: c.upagrahas.mandi ? {
        sign: c.upagrahas.mandi.sign, sign_idx: c.upagrahas.mandi.signIdx,
        within_deg: c.upagrahas.mandi.withinDeg, house: c.upagrahas.mandi.house,
        nakshatra: c.upagrahas.mandi.nakshatra, nakshatra_lord: c.upagrahas.mandi.nakshatraLord,
        pada: c.upagrahas.mandi.pada,
      } : null,
    } : null,
    gochara_now: (() => {
      const g = computeGochara(c, { now });
      if (!g) return null;
      return {
        ...
        transits: g.transits.map(t => ({
          name: t.name, sign: t.signName,
          house_from_moon: t.houseFromMoon, house_from_lagna: t.houseFromLagna,
          flavour: t.flavour,
...
export function exportVedicRaw(state) {
  ...
  downloadRawJson({
    name: state.birth?.name || 'kundali',
    system: 'vedic',
    payload: { birth: buildBirth(state.birth, state.timeUnknown), chart },
  });
}
