// Chinese BaZi birth form. Same fields as the other systems plus a
// gender toggle (needed for Luck Pillar direction).

import React, { useEffect, useRef, useState } from 'react';
import { useBazi, profileKey } from '../store.js';
...
export default function BirthForm() {
  const birth           = useBazi(s => s.birth);
  const setBirth        = useBazi(s => s.setBirth);
  const enter           = useBazi(s => s.enter);
  const profiles        = useBazi(s => s.profiles);
  const rememberProfile = useBazi(s => s.rememberProfile);
  const forgetProfile   = useBazi(s => s.forgetProfile);
  const clearProfiles   = useBazi(s => s.clearProfiles);
  const useProfile      = useBazi(s => s.useProfile);
  const timeUnknownGlobal = useBazi(s => s.timeUnknown);
  const setTimeUnknown    = useBazi(s => s.setTimeUnknown);
  const genderGlobal      = useBazi(s => s.gender);
      ...
      <div className="flex-1 w-full flex flex-col items-center justify-center p-6">
        <div className="w-full max-w-4xl">
          <div className="text-center mb-6">
            <div className="text-[#9b9bbd] text-[11px] tracking-[0.35em] uppercase">Ology · BaZi</div>
            <h1 className="font-serif text-[32px] tracking-[0.12em] mt-1 text-[#ff6a6a]">
              {profiles.length > 0 ? 'New Chart' : 'Cast the Four Pillars'}
            </h1>
            <p className="text-[13px] text-[#9b9bbd] mt-2 max-w-[560px] mx-auto leading-snug">
              Enter the moment and place of birth. BaZi reads four sexagenary
              pillars — Year, Month, Day, Hour — anchored on Lichun (the
              spring boundary, around Feb 4) and the 24 solar terms.
            </p>
          </div>

          <form onSubmit={submit} className="bg-[#0c0c18]/90 border border-white/10 rounded-lg p-6 shadow-2xl text-[#e6e6f0]">
            <div className="grid grid-cols-12 gap-x-4 gap-y-3">
              <Field label="Name (optional)" span={9}>
              ...
              <Field label="Gender (for Luck Pillar direction)" span={3}>
                <select value={form.gender} onChange={e => update('gender', e.target.value)} className="form-input" style={{ colorScheme: 'dark' }}>
              ...
              <Field label="Time of birth (local)" span={3}>
                ...
                <label className="mt-1 flex items-center gap-2 text-[11px] text-[#9b9bbd] cursor-pointer">
                  ...
                  Birth time unknown — Hour pillar won't be reliable.
                </label>
              </Field>
            ...
            <div className="mt-5">
              <button type="submit" disabled={!canSubmitFinal} className="w-full py-3 rounded-md text-[13px] tracking-[0.3em] uppercase bg-[#ff6a6a] text-[#0b0b15] font-medium hover:bg-[#ff9a9a] disabled:opacity-40 disabled:cursor-not-allowed transition-colors">
                Cast the Four Pillars
              </button>
            </div>
          </form>
...
function Field({ label, span = 1, children }) {
  const desktopCol = ({ 1:'sm:col-span-3', 2:'sm:col-span-6', 3:'sm:col-span-3', 4:'sm:col-span-4', 6:'sm:col-span-6', 9:'sm:col-span-9', 12:'sm:col-span-12' })[span] || 'sm:col-span-3';
  return (
